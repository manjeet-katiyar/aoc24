...
def dfs(graph, visited, n, m, x, y):
    ...
    for i in range(4):
        ...
        if is_inbound(n, m, nx, ny) and graph[nx][ny] == graph[x][y] and not visited[nx][ny]:
...
def count_edges(graph, n, m, x, y):
    a = (get_val(graph, n, m, x, y - 1), get_val(graph, n, m, x - 1, y - 1), get_val(graph, n, m, x - 1, y))
    b = (get_val(graph, n, m, x, y - 1), get_val(graph, n, m, x + 1, y - 1), get_val(graph, n, m, x + 1, y))
    c = (get_val(graph, n, m, x + 1, y), get_val(graph, n, m, x + 1, y + 1), get_val(graph, n, m, x, y + 1))
    d = (get_val(graph, n, m, x - 1, y), get_val(graph, n, m, x - 1, y + 1), get_val(graph, n, m, x, y + 1))
